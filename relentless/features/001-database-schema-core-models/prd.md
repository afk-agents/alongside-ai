...
- [ ] T001 [P] Verify Convex dev server runs without errors (`bunx convex dev`)
- [ ] T002 [P] Verify TypeScript compilation passes (`bunx tsc --noEmit`)
- [ ] T003 [P] Verify ESLint passes (`bun run lint`)
...
- [ ] `role` field uses union validator: `"admin" | "member" | "guest"`
- [ ] `profileStatus` field uses union validator: `"locked" | "unlocked" | "published"`
- [ ] All optional profile fields defined (displayName, bio, photoUrl, socialLinks, etc.)
- [ ] `socialLinks` nested object validator is correct
...
- [ ] T004 [US-001] Remove placeholder `numbers` table from `convex/schema.ts`
- [ ] T005 [US-001] Add `profiles` table definition with all fields in `convex/schema.ts`
- [ ] T006 [US-001] Add indexes for `profiles` table: `by_userId`, `by_role`, `by_profileStatus`
...
- [ ] T010 Run `bunx tsc --noEmit` to verify TypeScript compilation
...
**Acceptance Criteria:**
- [ ] `projects` table defined with all required fields
- [ ] `authorId` references `profiles` table
- [ ] `tags` references `tags` table as array
...
- [ ] T021 [P] [US-005] Add `projects` table definition in `convex/schema.ts`
- [ ] T022 [P] [US-005] Add indexes for `projects` table
